...
public class LimiteController {
    ...
    @GetMapping("/lista")
    public ResponseEntity<List<Limite>> lista(){
        List<Limite> listartudo = Service.listartudo();
        return ResponseEntity.ok(listartudo);
    }
    @GetMapping("/lista/id/{id}")
    public ResponseEntity<?> listaId(@PathVariable(value = "id") Long id){
        Limite listarid = Repository.findById(id).orElse(null);
        return listarid == null
                ? ResponseEntity.badRequest().body(" <<ERRO>>: valor nao encontrado.")
                : ResponseEntity.ok(listarid);
    }
    @GetMapping("/lista/ativo/{ativo}")
    public ResponseEntity<List<Limite>> listaAtivo(@PathVariable boolean ativo) {
    ...
    public ResponseEntity<String> delete(@PathVariable Long id){
        Optional<Limite> deletarId = Repository.findById(id);
    ...
    public ResponseEntity<?> atualizar( @PathVariable Long id, @RequestBody Limite atualizarId) {
        try {
